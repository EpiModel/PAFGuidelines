...
# set_n <- 6
for (set_n in seq_len(6)) {
  test_simulation <- set_n == 1
  ...
  slurm_wf_Map(
    ...
    MoreArgs = list(
      orig = orig, param = param, init = init, control = control,
      info = info
    )
  )
  ...
  scp_send_script <- c(
    ...
    "",
    paste0(
      "ssh ", info$ssh_host, " \"mkdir -p '", info$ssh_host, ":",
      fs::path(paths$ssh_proj, paths$jobs_dir), "'\""
    ),
    paste0(
      "rsync -vr --exclude '", "out/*", "' '",
      paths$local_job_dir, "' '",
  ...
  write(job_name,
    file = fs::path(paths$local_out, paths$jobs_dir, "last_jobs"),
    append = TRUE
  )
}
