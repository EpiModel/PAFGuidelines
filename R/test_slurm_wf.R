...
batch_per_set <- 2     # How many sim_per_batch replications to do per parameter
...
infos$ssh_host <- ssh_host
infos$root_dir <- fs::path(paths$jobs_dir, job_name, paths$slurm_wf)
infos$param_proposals <- param_proposals
infos$unique_proposals <- unique_proposals

slurm_wf_tmpl_dir("inst/slurm_wf/", infos$root_dir, force = TRUE)
...
if (test_simulation) {
  ...
  calibration_fun(
    ...
    orig, param, init, control_test, infos
  )
}
...
slurm_wf_Map(
  ...
  MoreArgs = list(orig = orig, param = param, init = init, control = control,
                  infos = infos),
  with_objects = list(calculate_targets = calculate_targets)
)

gather_resources <- slurm_resources
gather_resources$n_cpus <- 1
gather_resources$memory <- 8 * 1e3

slurm_wf_do.call(
  infos$root_dir,
  resources = gather_resources,
  what = gather_calibration_fun,
  args = list(infos = infos)
)

# Create out dir and save params
fs::dir_create(fs::path(paths$local_out, paths$jobs_dir))
saveRDS(infos, fs::path(paths$remote_job_dir, "job_infos.rds"))
...
scp_send_script <- c(
  "#!/bin/sh",
  "",
  paste0("ssh ", infos$ssh_host, " \"mkdir -p '", infos$ssh_host, ":",
         fs::path(paths$ssh_proj, paths$jobs_dir), "'\""),
  paste0("rsync -vr --exclude '", "out/*", "' '",
         paths$local_job_dir, "' '",
         infos$ssh_host, ":", fs::path(paths$ssh_proj, paths$jobs_dir, "'"))
  )
...
scp_get_script <- c(
  ...
  "",
  paste0("rsync -vur '",
         infos$ssh_host, ":", fs::path(paths$ssh_job_dir, paths$slurm_out),
         "' '", paths$local_job_dir, "'")
)
